...
fn main() {
    ...
    for path in paths {
        ...
        if !x.metadata().unwrap().is_file() {
            let loc = x.path();
            let loc2 = loc.display();
            println!("Skipping folder: '{loc2}'");
            continue;
        }
        let file_content = fs::read_to_string(x.path()).expect("Failed to read metafile");
    ...
    let folder_name = yt_metadata_folder.split("\\").last().unwrap();
    let x = output.join(format!("{folder_name}.csv"));
    let text = String::from_utf8(wtr.into_inner().unwrap()).unwrap();
    fs::write(x, text).expect("Failed to save results into csv file. Check output location");
    println!("Done!")
}
